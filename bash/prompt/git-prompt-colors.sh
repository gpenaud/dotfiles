...
define_git_prompt_colors() {
  ...
  if [ "$STY" != "" ]; then
     ScreenId=$(echo $STY | cut -d'.' -f 1)
     ScreenName=$(echo $STY | cut -d'.' -f 2)

     case $(id -u) in
       1000)
         ScreenState=" ${ResetColor}[[ ${Green}${ScreenId}${ResetColor} ${ScreenName} ]]"
         ;;
      1001)
         ScreenState=" ${ResetColor}[[ ${Green}${ScreenId}${ResetColor} ${ScreenName} ]]"
         ;;
       0)
         ScreenState=" ${ResetColor}[[ ${Magenta}${ScreenId}${ResetColor} ${ScreenName} ]]"
         ;;
     esac
  fi
